					...
					<ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
						...
						<li>
							<div class="flex items-center">
								...
								<span class="ml-1 text-gray-400 dark:text-gray-500 md:ml-2" aria-current="page">{{ __("Create") }}</span>
							</div>
						</li>
					</ol>
				</nav>
				<h1 class="text-xl font-semibold text-gray-900 dark:text-white sm:text-2xl">{{ __("Create New Blog") }}</h1>
			</div>
		</div>
	</div>
			...
			<form action="{{ route("admin.blogs.store") }}" method="POST" enctype="multipart/form-data" class="space-y-6">
				...
				<div>
					...
					<input type="text" name="title" id="title"
						...
						placeholder="{{ __("Enter blog title") }}" value="{{ old("title") }}" required>
					@error("title")
				...
				<div>
					...
					<textarea name="excerpt" id="excerpt" rows="3"
					 class="focus:ring-primary-600 focus:border-primary-600 block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500"
					 placeholder="{{ __("Short description of the blog") }}">{{ old("excerpt") }}</textarea>
					@error("excerpt")
						<p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
					@enderror
				</div>
				...
				<div>
					<label for="featured_image"
						class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">{{ __("Featured Image") }}</label>
